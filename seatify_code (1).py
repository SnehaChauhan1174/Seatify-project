...
import mysql.connector #for connecting to sql and encryption of passwords
...
mydb=mysql.connector.connect(host="localhost",user="root",passwd="qwerty!@#123**")
...
"""Create a new window and close the previous one."""
def checkout():
...
def seatmatrix():
    ...
    # defining function for selecting seats

    #from tkinter import PhotoImage, DISABLED
...
def login_pg():
    ...
    class login:
        ...
        def veri(self,primary_key):
         try:
          con = pymysql.connect(host="localhost", user="root", passwd="qwerty!@#123**", db="userdata")
          cursor = con.cursor()
  ...
  class signup:
    ...
    def connect_database(self):
        if self.txt_user.get() == '' or self.txt_email.get() == '' or self.txt_pass.get() == '':
            ...
            try:
                con = pymysql.connect(host="localhost", user="root", passwd="qwerty!@#123**")
                mycursor = con.cursor()
                ...
                mycursor.execute(query)
                #id int to_increment primary key
